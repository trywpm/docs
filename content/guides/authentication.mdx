...
- [`wpm auth`](../reference/cli/auth): the parent command for `login` and
  `logout`.
- [`wpm whoami`](../reference/cli/whoami): the canonical "am I logged in?"
  check.
- [wpm registry](../fundamentals/registry): visibility, dist tags, and what
  the registry does with your identity.
- [CI/CD guide](ci): worked examples for running wpm from build pipelines.
