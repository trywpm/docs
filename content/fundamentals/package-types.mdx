...
- [`wpm.json` reference](wpm-json): the schema where `type` is declared.
- [`wpm init`](../reference/cli/init): the detection logic for existing
  projects.
- [`wpm install`](../reference/cli/install): how the extracted path is computed
  at install time.
